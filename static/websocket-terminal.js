...
class WebSocketTerminal {
    ...
    // Thay thế hàm connect() hiện tại bằng hàm này
connect() {
    ...
    try {
        // URL của server (không cần /ws/terminal)
        const wsUrl = `<span class="math-inline">\{window\.location\.protocol\}//</span>{window.location.host}`;
        console.log(`📡 Connecting to Socket.IO server: ${wsUrl}`);

        // Sử dụng io() thay vì new WebSocket()
        this.ws = io(wsUrl, {
        ...
        this.setupWebSocketHandlers(); // Giữ nguyên hàm setup handlers
